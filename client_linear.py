...
x_train, y_train = create_linear_training_data()
# x_train, y_train = create_folded_training_data()
# x_train, y_train = load_dataset_flattened(train=True, dataset='Fashion-MNIST', download=True)
...
with Timer('Training time'):
    ...
    for j in range(epochs_to_train):
        ...
        for i in range(batch_size, num_train_samples + 1, batch_size):
            ...
            #     train_accuracy = num_correct_preds / samples_per_accuracy_check
            #     print("training accuracy in epoch", str(j), "by batch", str(i / batch_size) + ":",  train_accuracy)
...
_, loss = network.forward(x_test, y_test)
print(f"Loss on test data: {loss/60000}")
print("M*W:", torch.matmul(W_1.output, W_0.output))

# Loss on test data: 1.045689600687183e-06
# M*W: tensor([[-0.0240, -0.9991],
#         [ 1.0190,  0.0112]])
